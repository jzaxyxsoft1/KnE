...
function getChildren(ds,ihi){
    ihi.items=[];
    _.each(ds,function (i,idx){
        if(i.Parent.Value== ihi._id){
           var p = ds.splice(idx,1)[0];
            getChildren(ds,p);
            ihi.items.push(p);
        }
    });
}
exports.getihi=function (req,res){
    var tp= req.query['tp'];
    var id=req.query['id'];
...
exports.postsave = function (req, res) {
    ...
    if (obj._id) {
        ...
        Svc.update(tp, {_id: oi}, up, req.currentUser, function (e) {
            res.json({msg: e == null, error: e, ID: oi});
        });
    }
    else {
        Svc.insert(tp, obj, req.currentUser, function (e, ds) {
